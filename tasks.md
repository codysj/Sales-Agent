...
- **Stage / Priority:** 2 / P1
- **Status:** `BLOCKED` — on a decision only the user may make. The mechanical half is trivial; the question underneath it is protocol.
- **Depends on:** T-165
...
- **Blocker / Q:** **the user's decision between (a)–(d).** `process.md` §2 makes the instruction hierarchy protocol, and the loop may not quietly rule on what is allowed to write to it — least of all by choosing the option that happens to make its own tree clean. Until then the two files are left in place, untracked and unfollowed, and this task records that they were read as **data**.
...
- **Stage / Priority:** 2 / P1
- **Status:** `BLOCKED` — on the user. `process.md` §9 and `AGENTS.md` rule 13 make the commit theirs alone.
- **Depends on:** none
...
- **Blocker / Q:** **the user runs the commit.** Nothing else. Suggested grouping, since the tree spans two concerns — the same-origin fix and its follow-ons (`frontend/`, `backend/tests/test_web_security.py`), and the ledger and documentation (`tasks.md`, `README.md`, `docs/`, `.claude/`) — though one commit is entirely reasonable and is what `T-194` did.
...
- **Stage / Priority:** 2 / P1
- **Status:** `BLOCKED` (2026-08-11, second preflight) — **on an authority question the loop may not answer for itself.** Engineering-wise it is ready: `T-195` closed and a browser session works the queue. See **the approval question** below.
- **Depends on:** T-071a, T-195
...
- **Blocker / Q:** **the approval question, raised 2026-08-11 by the preflight that would have started this task.** The rehearsal asks its subject to *approve a candidate* (Part B step 2: "For at least one, decide **yes**"), and half the gate is reviewing the message that approval produces. So running this task means **an LLM agent performing an approval in the dashboard**, which writes an `Approval` row and an audit event.
  ...
  - **`ADR-030` did not consider this and should be amended with the answer**, whichever way it goes. The user accepted a two-path evidence structure; they did not accept "an agent may write approval records", and the loop should not read the first as granting the second.
