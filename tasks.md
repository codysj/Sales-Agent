...
- **Stage / Priority:** 2 / P0
- **Status:** `IN_PROGRESS` (claimed 2026-08-11) — built and test-proven; held open for the browser pass.
...
- **Blocker / Q:** none for the code. The end-to-end browser check is blocked on the **Docker daemon**, which went down mid-verification.
- **Progress (2026-08-11) — built and test-proven; the browser check is outstanding.**
...
- **Remaining work:** the end-to-end browser pass, and `uv run pytest -q`. Both need PostgreSQL and **Docker went down partway through verification** — a throwaway `verify_t205` database was built and the servers started, but sign-in returned `500` with `psycopg.errors.ConnectionTimeout` once the daemon stopped. No backend code changed, so the backend suite is a formality here; the browser pass is not, and it is the check that found this defect in the first place. `docker compose up -d db`, then approve a candidate, let the worker draft, and use the new control.
