...
- **Stage / Priority:** 2 / P1
- **Status:** `BLOCKED` — on a person, not on code. `T-071a` closed 2026-08-01, so every engineering dependency is met.
- **Depends on:** T-071a
...
- **Blocker / Q:** a non-engineer reviewer, which this loop cannot be. **Everything else is done:** [docs/stage2-exit-evidence.md](docs/stage2-exit-evidence.md) carries the executed script, and its §5 lists exactly what the rehearsal must record. `Q-005` (who may approve) is worth settling before it is scheduled, because the rehearsal will surface it either way.
