...
- **Stage / Priority:** 2 / P1
- **Status:** `BLOCKED` — on a person **and now on `T-195`**. `T-071a` closed 2026-08-01; the engineering dependency that reopened is the dashboard being usable in a browser at all.
...
- **Blocker / Q:** two now. **`T-195`** — a reviewer cannot get past sign-in in a real browser, so there is nothing to rehearse until it is fixed. And **a non-engineer reviewer**, which this loop cannot be. `Q-005` (who may approve) is worth settling before it is scheduled, because the rehearsal will surface it either way.
...
- **Stage / Priority:** 2 / P1
- **Status:** `BLOCKED` — on `T-195` only. No person, no schedule, and no stakeholder decision stands in front of it.
- **Depends on:** T-071a, T-195
...
- **Blocker / Q:** **`T-195`**. An agent driving a browser hits exactly the wall a person does. Nothing else — this is the one path to a gate decision that needs no calendar.
