...
| [ADR-021](ADR-021-frontend-toolchain-defaults.md) | ACCEPTED (amended) | Frontend: npm + committed lockfile, Next.js 16 App Router, TypeScript 5.9 beyond `strict`, type-aware ESLint, Vitest; no styling until a screen needs it. Amended 2026-07-31: TS 5, not 6, because `openapi-typescript` requires it. The styling row is **superseded 2026-08-15 by ADR-031**, when the screen that needed it arrived. |
...
| [ADR-031](ADR-031-the-review-desk-has-a-visual-design.md) | ACCEPTED | One stylesheet of element and attribute selectors, no `className` anywhere: two typographic registers (the message as correspondence, everything else as record), the message paired beside what it cites, and the palette spent on one signal. **Nothing may imply entailment** — citation is shown as citation, because §19.3 is not built. Answers the styling question ADR-021 deferred, and corrects the `T-062` pointer that deferral was addressed to. |
